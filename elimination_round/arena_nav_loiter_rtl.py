...
#  LOGGING
# ══════════════════════════════════════════════════════════════
def log(m):  rospy.loginfo(f"[ARENA-RTL] {m}")
...
def run_mission():
    ...
    log(f"Shift: {SHIFT_PITCH_PWM}/{T_SHIFT}s  Yaw180: {YAW_180_PWM}/{YAW_180_TIME_S}s  RTL wait: {RTL_WAIT_S}s")
    ...
    # Monitor yellow-front in background while RTL runs.
    # If yellow front is detected first → cancel RTL, hover, then land normally.
    log(f"Monitoring yellow-front during RTL (up to {RTL_WAIT_S}s)...")
    t0 = time.time()
    yellow_intercept = False
    rate = rospy.Rate(10)
    while _ok() and (time.time() - t0) < RTL_WAIT_S:
        if yellow_active("front"):
            log("Yellow 'front' detected during RTL! Cancelling RTL → hover → land.")
            yellow_intercept = True
            break
        rate.sleep()

    if yellow_intercept:
        # Switch to hover mode chosen at mission start
        log(f"Hovering {YELLOW_STOP_HOVER_S}s before landing (yellow intercept).")
        do_hover(YELLOW_STOP_HOVER_S, hover_mode)
    else:
        log("RTL timeout reached — proceeding to LAND (safety).")
...
def main():
    ...
    rospy.init_node("arena_nav_loiter_rtl", anonymous=False)

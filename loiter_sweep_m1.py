#!/usr/bin/env python3
"""
loiter_sweep_m1.py  —  ROS1 Melodic / MAVROS
=============================================
Mission 1: RC takeoff → roll/pitch sweep in LOITER → return pitch → FLOWHOLD land
...
After all loops:
  Return pitch RETURN_PITCH_PWM for RETURN_T s → FLOWHOLD → LAND (20s)

Safety: 'l' key → land, Ctrl+C → land+disarm, VIO divergence guard
"""
...
TAKEOFF_PWM_STEP   = 10
TAKEOFF_TICK_S     = 0.3
GROUND_ALT_OFFSET  = 0.10   # rangefinder offset to detect liftoff
TAKEOFF_TARGET_OFF = 0.35   # target alt above ground (m) — change for higher hover

# Number of sweep loops
N_LOOPS = 1

# Per-loop motion
ROLL_R_PWM   = 1530   # roll right channel value
T1           = 5.0    # roll right duration (s)

PITCH_F_PWM  = 1470   # pitch forward channel value
T2           = 5.0    # pitch forward duration (s)

ROLL_L_PWM   = 1470   # roll left channel value
T3           = 5.0    # roll left duration (s)

# Shift pitch (applied BETWEEN loops, not after last)
SHIFT_PITCH_PWM = 1470
T4              = 3.0    # shift duration (s)

# Hover between commands (neutral or flowhold — asked at runtime)
HOVER_S = 3.0   # hover duration between each motion command

# Return pitch (after all loops, before landing)
RETURN_PITCH_PWM = 1530   # e.g. pitch back to return
RETURN_T         = 5.0    # return pitch duration (s)

# Land
LAND_WAIT_S = 20.0   # wait after LAND mode before disarming
...
#  LOGGING
# ──────────────────────────────────────────────────────────────
def log(m):  rospy.loginfo(f"[SWEEP] {m}")
...
def arm(do_arm):
    ...
    try:
        ...
        log("Arming ACCEPTED" if (do_arm and res.success) else
            "Disarming ACCEPTED" if res.success else f"Arm cmd REJECTED (code:{res.result})")
        return res.success
    except Exception as e:
...
def sleep_check(secs):
    """Sleep secs; returns False if abort or land triggered."""
    t0 = time.time(); r = rospy.Rate(10)
...
def rc_takeoff(arm_mode, wait_str):
    ...
    warn("Ramp exhausted without target alt. Continuing.")
...
# ──────────────────────────────────────────────────────────────
def run_mission1():
    log("="*40); log("LOITER SWEEP MISSION 1"); log("="*40)
    log(f"Loops: {N_LOOPS} | Roll-R: {ROLL_R_PWM}/{T1}s | Pitch-F: {PITCH_F_PWM}/{T2}s | Roll-L: {ROLL_L_PWM}/{T3}s")
    log(f"Shift: {SHIFT_PITCH_PWM}/{T4}s | Hover between: {HOVER_S}s | Return: {RETURN_PITCH_PWM}/{RETURN_T}s")
    ...
    start_rc_heartbeat()
    set_rc_hb(1500,1500,1500,1500)  # clear takeoff residual
    ...
    for loop in range(1, N_LOOPS+1):
        ...
        if not sleep_check(T1): do_land(); return
        ...
        if not sleep_check(T2): do_land(); return
        ...
        if not sleep_check(T3): do_land(); return

        # Hover after roll left
        if not do_hover(HOVER_S, hover_mode): do_land(); return

        # Shift pitch between loops (not after last loop)
        if loop < N_LOOPS:
            log(f"Shift pitch ({SHIFT_PITCH_PWM}) for {T4}s")
            set_rc_hb(roll=1500, pitch=SHIFT_PITCH_PWM, throttle=1500, yaw=1500)
            if not sleep_check(T4): do_land(); return
            if not do_hover(HOVER_S, hover_mode): do_land(); return
    ...
    # ── RETURN PITCH ──
    if abort_flag.is_set() or land_flag.is_set(): do_land(); return
    log(f"Return pitch ({RETURN_PITCH_PWM}) for {RETURN_T}s")
    set_rc_hb(roll=1500, pitch=RETURN_PITCH_PWM, throttle=1500, yaw=1500)
    if not sleep_check(RETURN_T): do_land(); return

    # Neutral before landing
    set_rc_hb(1500,1500,1500,1500)
    sleep_check(1.0)
...
def main():
    ...
    rospy.init_node("loiter_sweep_m1", anonymous=False)

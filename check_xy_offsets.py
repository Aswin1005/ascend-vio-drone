...
class XYOffsetChecker(object):
    def __init__(self):
        ...
        self.bridge = None  # cv_bridge not used — direct numpy decode instead
        ...
        # Low-Pass Filter States (smooths pixel noise between frames)
        self.filtered_x = None
        self.filtered_y = None
        self.filter_alpha = 0.15  # 0=no update, 1=no filter; 0.15 = heavy smoothing
        ...
        rospy.loginfo("Corrected robust XY Offset Checker (INFRARED) started.")
    ...
    def image_callback(self, msg):
        ...
        # Direct numpy decode — no cv_bridge needed (works Python 2 & 3)
        try:
            if msg.encoding in ('mono8', '8UC1'):
                ...
                cv_img = np.frombuffer(msg.data, dtype=np.uint16).reshape(msg.height, msg.width)
                cv_img = (cv_img >> 8).astype(np.uint8)  # scale to 8-bit
            else:
                rospy.logwarn_throttle(10, "Unknown encoding: %s" % msg.encoding)
                return
        except Exception as e:
            rospy.logerr("Image decode error: %s" % str(e))
            return
        ...
        if len(calculated_offsets) > 0:
            ...
            # Publish filtered offsets to ROS topics (in cm)
            self.pub_status.publish(Bool(data=True))
            self.pub_x_offset.publish(Float32(data=self.filtered_x * 100.0))
            self.pub_y_offset.publish(Float32(data=self.filtered_y * 100.0))
            ...
            # No markers: reset filter and publish status=False
            self.filtered_x = None
            self.filtered_y = None
